      ...
      {rows.map((r) => (
        <Row key={r.id} title={r.chefName ?? "Chef"} sub={`${r.companyName ?? "Klant"} · ${formatShiftRole(r.roleNeeded)} · ${dateLabel(r.startsAt)}`}>
          ...
          <span className="flex shrink-0 flex-wrap items-center gap-1.5">
            {r.chefPhone && (
              <a href={`tel:${r.chefPhone.replace(/[^+\d]/g, "")}`} className="rounded-full border border-ink-200 bg-white px-2.5 py-1 font-ui text-[10px] font-medium uppercase tracking-[0.12em] text-ink-700 hover:border-burgundy hover:text-burgundy">
                Bel
              </a>
            )}
            {r.chefPhone && (
