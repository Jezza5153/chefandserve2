...
import { submitClientRequestAction } from "./actions";
...
export function ClientRequestForm({ form }: { form: FormDTO }) {
  ...
  function submit() {
    ...
    start(async () => {
      const res = await submitClientRequestAction({ ...values, __hp: hp });
      if (res.ok) {
        setDone(true);
      } else if (res.error === "rate_limited") {
        setFlash("Je hebt te veel aanvragen verstuurd. Probeer het later opnieuw of bel ons.");
  ...
  if (done) {
    return (
      <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-6 text-center">
        <p className="font-serif text-xl text-emerald-900">✓ Bedankt voor je aanvraag!</p>
        <p className="mt-2 text-sm text-emerald-800">
          Maarten of Gina kijkt er binnen 4 werkuren naar en koppelt je aan de juiste professionals.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-ink-200 bg-white p-6">
      ...
      <p className="mt-3 text-center text-xs text-ink-400">
        We gebruiken je gegevens alleen om contact op te nemen over je aanvraag.
      </p>
    </div>
  );
