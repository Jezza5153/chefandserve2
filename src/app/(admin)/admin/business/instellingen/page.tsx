...
export default async function BedrijfsinstellingenPage({
  ...
  const availabilityRemindersOn = await getFlag(SETTING_KEYS.availabilityReminders);
  ...
  async function saveAvailabilityReminders(formData: FormData) {
    "use server";
    const s = await requirePermission("settings", "write");
    const enabled = formData.get("enabled") === "on";
    await setFlag(SETTING_KEYS.availabilityReminders, enabled, s.user.id);
    await recordAuditFromRequest({
      userId: s.user.id,
      action: "settings.update",
      resource: "business_settings",
      resourceId: SETTING_KEYS.availabilityReminders,
      after: { key: SETTING_KEYS.availabilityReminders, enabled },
    });
    redirect("/admin/business/instellingen?ok=availability_reminders");
  }
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      {/* ───── Automatisering: beschikbaarheids-herinnering ───── */}
      <section className="mt-6 rounded-lg border border-ink-200 bg-white p-5">
        <h2 className="font-ui text-[11px] uppercase tracking-[0.18em] text-burgundy">Beschikbaarheid</h2>
        <form action={saveAvailabilityReminders} className="mt-3">
          <label className="flex items-start gap-3">
            <input
              type="checkbox"
              name="enabled"
              defaultChecked={availabilityRemindersOn}
              className="mt-1 h-4 w-4 rounded border-ink-300 text-burgundy focus:ring-burgundy"
            />
            <span>
              ...
              <span className="mt-1 block font-ui text-[11px] text-ink-400">
                Status nu: {availabilityRemindersOn ? "aan" : "uit"} · draait donderdag om 09:00.
              </span>
            </span>
          </label>
          <button
            type="submit"
            className="mt-3 rounded bg-burgundy px-4 py-2 font-ui text-[11px] font-semibold uppercase tracking-[0.12em] text-white hover:bg-burgundy-900"
          >
            Opslaan
          </button>
        </form>
