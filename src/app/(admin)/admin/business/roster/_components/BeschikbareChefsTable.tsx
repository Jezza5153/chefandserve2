          ...
          <tbody>
            {rows.map((c) => (
              <tr key={c.id} className="border-b border-ink-50 last:border-0">
                ...
                <td className="whitespace-nowrap px-2 py-2.5 text-ink-500">Hele dag</td>
                <td className="truncate px-2 py-2.5 text-ink-600">{c.locatie ?? "—"}</td>
                <td className="truncate px-4 py-2.5 text-ink-600">{c.voorkeur ?? "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
