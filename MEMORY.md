...
**AI assistant**: `AI_ENABLED=true` · `OPENAI_API_KEY` (⚠ rotation pending — owner) · `OPENAI_MODEL=gpt-5.4` · `AI_CONFIRM_SECRET` (≥32 chars, signs confirm tokens) · `OPENAI_PRICE_INPUT_PER_1M` / `OPENAI_PRICE_OUTPUT_PER_1M` / `OPENAI_PRICE_CURRENCY` (optional — override the built-in gpt-5.4 rate shown on the AI-tokens card). `OPENAI_API_KEY` also activates the `embedding-refresh` worker (RAG).
