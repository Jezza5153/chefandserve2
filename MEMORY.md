...
| PR-PLANBORD-7 | **Reistijd-hint in de match-rail.** Elke gerangschikte kandidaat toont nu zijn **afstand tot de locatie** (`estimateTravel`, chef-coörd → shift-coörd) — zodat de planner een nabije good-enough chef kan verkiezen boven een verre topscore. Afstand-only (modus-onafhankelijk). | ✅ verified (type-check+lint+build groen) — op `feat/planbord-travel` |
