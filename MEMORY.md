...
| PA-V1 | Owner AI assistant: channel-agnostic agent loop + OpenAI brain (native tool-call threading) · 21 tools (8 read / 7 act / 6 personal) · owner-gated `/api/ai/chat` + dashboard chat widget · rate-limit `ai_chat_user` (30/min) · reminders + memory as jsonb bags in `business_settings` · smokes `smoke-ai-{spine,brain,tools,safety}` | ✅ live (no migration). ⚠ OpenAI **test key NEEDS ROTATION** (exposed in chat) · `AI_CONFIRM_SECRET` set · WhatsApp channel ON HOLD |
