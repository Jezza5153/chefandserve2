...
| Oud systeem (ShiftManager) | **Draait nog parallel** — boekte 381 diensten in juli 2026. Het archief in `legacy_ops_months`/`legacy_client_totals` loopt 2022-01 t/m 2026-08 (de laatste maand is nog niet afgerond — daar staat alleen wat al geboekt is). Een klant die daar boekt maar hier ontbreekt is **niet overgezet**, geen verloren klant: `ops.history` (view per_klant) geeft de status. Naam-koppeling loopt via `domain/legacy-match.ts` op **insluiting**, nooit op gelijkenis — losser matchen koppelde het Amstel Hotel aan Van der Valk. Tel de twee systemen nooit bij elkaar op. |
| Nieuwe vlaggen (allemaal UIT) | `BULK_MESSAGE_ENABLED` (groepsbericht aan chefs) · `INVOICE_REMINDERS_ENABLED` (aanmaningen; `?dryRun=1` op `/api/cron/invoice-reminders` toont wie er aangemaand zou worden zonder te versturen). Toeslagen hebben géén vlag maar zijn leeg: `/admin/business/toeslagen` rekent niets tot er een regel aanstaat. |
