...
**Tool registry (`src/lib/ai/tools/index.ts`) — 39 tools (23 read / 10 act / 6 personal):**
...
- **Ownership scope, not RBAC.** `AiActor` gained a `subject: {kind, entityId}`. `resolveChefActor`/`resolveClientActor(userId)` resolve the entity from `chefs.userId`/`clients.userId` (the "auth IS the lookup" rule) and stamp it as the subject. The portal tools (chef: `mijn.diensten`/`mijn.uren`/`mijn.profiel` · klant: `onze.diensten`/`onze.uren`/`onze.aanvragen`, in `tools/chef-self.ts` + `tools/client-self.ts`) are `permission:null` + `risk:'read'` and key EVERY query off `ctx.actor.subject.entityId` — the model never supplies an id, and the tool input schemas accept none, so a caller can't be steered to another tenant's data.
...
The owner AI assistant is **live** (PA-V1 + 2026-06 expansion + notes-RAG + Klant 360): 39 tools in
