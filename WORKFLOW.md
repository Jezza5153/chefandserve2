...
**Companion to:** `MEMORY.md` (state) · `FEATURES.md` (what the system can do) · `AI.md` (the AI layer) · `docs/STATE.generated.md` (all counts) · `docs/ai/` (workflow playbooks)
...
**Tool registry (`src/lib/ai/tools/index.ts`) — 65 owner-assistant tools (41 read / 18 act / 6 personal); the registry also carries 3 tools from the parallel PR-INTEL workstream (`chefs`/`clients.intel_snapshot` · `match.intel`) → 68 total in the gate.**
- *read:* `business.overview` · `shifts.open_soon`/`find`/`detail` (stand van één dienst: bezetting + team + uren-status) · `chefs.find` · `clients.find` · `insights.leaderboards` · `integrations.health` · `hours.list_awaiting_approval` · `hours.detail` (één regel: gepland vs werkelijk + marge + markeringen + advies) · `chefs.list_profile_changes` · `chefs.work_summary`/`feedback`/`trends` · `chefs.availability` (doorgegeven beschikbaarheid van één chef) · `chefs.history_at_client` (track record van chef bij één klant — plaatsingsbeslissing) · `intake.list` (intake-inbox: nieuwe chef-/klant-aanmeldingen) · `onboarding.missing` (wie mist verplichte AVG/onboarding-velden — labels only, nooit waarden) · `clients.shift_requests` (klant wijzigings-/annuleringsverzoeken op diensten) · `clients.history` · `clients.health` (Klant 360 verdict) · `roster.overview` · `planner.cockpit` · `risks.scan` (vooruitblik: onderbezetting + verlopende papieren bij ingeroosterde chefs + vastgelopen uren) · `shifts.suggest_chefs` · `shifts.margin` · `contacts.timeline` · `chefs`/`clients.semantic_search` · `knowledge.search` · `reports.business_kpi` / `reports.chefs` / `reports.clients` (genereren **PDF-rapporten** — KPI-dashboard, chef-prestaties, klant-omzet — met grafiek + uitleg → R2-downloadlink; `@react-pdf/renderer`, gedeelde `reports/render.tsx`) · `briefing.daily` (dagstart: gisteren-recap + vandaag-forecast) · `audit.search` · `documents.list_for_chef` · `documents.expiring` (all chefs, soonest-first) · `privacy.list_requests` · `email.status` · `payroll.read` (the oversight cluster = Restricted tool-only data, metadata/aggregate-only)
- *act (confirm-gated):* `hours.approve`/`reject`/`send_reminder` · `placements.propose`/`confirm`/`cancel` · `roster.publish`/`autofill`/`copy_last_week` · `email.send` · `chefs.approve`/`reject_profile_change` · `chefs.send_availability_reminder` · `contacts.log` (leg een contactmoment vast bij chef/klant — schrijft contact_logs) · `clients.decide_shift_request` (keur klant wijzigings-/annuleringsverzoek goed/af — wrapt `decideShiftChangeRequest`, atomair + klant-mail; tier financial) · `placements.reply` (plaats een bericht in de plaatsing-gesprekslijn — klant-zichtbaar of intern; wrapt `addPlacementComment`) · `intake.convert` (zet een intake-aanmelding om naar chef/klant-profiel; wrapt `convert{Chef,Client}Submission`, idempotent) · `onboarding.request_missing` (mail chef(s) PRECIES welke AVG/onboarding-velden ze nog moeten aanvullen — labels only; wrapt `createProfileDataRequest`; bulk = confirm-gated)
